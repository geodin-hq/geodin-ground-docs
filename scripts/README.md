...
./scripts/check-duplication.sh public # narrow scope
```
...
ROOT=public ./scripts/check-asset-integrity.sh   # narrow scope
```
...
```bash
python3 scripts/check-text-similarity.py public
python3 scripts/check-text-similarity.py public --output ~/GitHub/geodin-docs-internal/insights/AUDIT-text-similarity.md
python3 scripts/check-text-similarity.py public --page-threshold 0.70 --block-threshold 0.85 --quiet
```
...
python3 scripts/check-relative-links.py public  # narrow scope
```
...
```bash
./scripts/check-typography.sh public
```
